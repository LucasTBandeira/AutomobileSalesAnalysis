...
app.layout = html.Div(style={'backgroundColor': '#1E1E1E', 'color': 'white'}, children=[
    ...
    html.Div([
        html.Label("Select Statistics:", style={'padding': '3px', 'font-size': '20px', 'color': 'white'}),
        dcc.Dropdown(
            ...
            style={'padding': '3px', 'font-size': '20px', 'text-align-last': 'center', 'color': 'black'}
        )
    ],style={'width': '50%'},),
...
def recession_report():
    ...
    chart1 = dcc.Graph(figure=px.line(yearly_rec, x='Year', y='Automobile_Sales', title="Automobile Sales Over Time").update_layout(template="plotly_dark"), style={'width': '100%'})
    chart2 = dcc.Graph(figure=px.bar(avg_sales_type, x='Vehicle_Type', y='Automobile_Sales', title="Automobile Sales by Vehicle Type", color='Vehicle_Type').update_layout(template="plotly_dark"), style={'width': '100%'})
    chart3 = dcc.Graph(figure=px.pie(total_exp, names='Vehicle_Type', values='Advertising_Expenditure', title="Advertising Expenditure by Vehicle Type").update_layout(template="plotly_dark"), style={'width': '100%'})
    chart4 = dcc.Graph(figure=px.bar(unemployed_sales, x='unemployment_rate', y='Automobile_Sales', title="Unemployment Rate Effect on Automobile Sales by Vehicle Type", color='Vehicle_Type').update_layout(template="plotly_dark"), style={'width': '100%'})
